...
use wasmer::{
    ...
    wat2wasm, BaseTunables, Instance, Memory, MemoryType, Module, Pages, Store, TableType, Target, Tunables,
};
...
/// 
/// After adjusting the memory limits, it delegates all other logic to the base tunables. 
#[derive(MemoryUsage)]
pub struct LimitingTunables<T: Tunables> {
    /// The maximum a linear memory is allowed to be (in Wasm pages, 64 KB each).
...
impl<T: Tunables> LimitingTunables<T> {
    ...
    /// Takes an input memory type as requestd by the guest and sets 
    /// a maximum if missing. The resulting memory type is final if vaild. 
    ...
    /// Ensure the a given memory type does not exceed the memory limit. 
    ...
    fn validate_memory(&self, ty: &MemoryType) -> Result<(), MemoryError> {
        ...
        if let Some(max) = ty.maximum {
            if max > self.limit {
                return Err(MemoryError::Generic("Maximum exceeds the allowed memory limit".to_string(), ));
            }
        } else {
