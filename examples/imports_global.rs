...
fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  // The Wasm module only imports some globals.
  // We'll have to interact with them either using the Global API or exported functions. 
  ...
  // Trying to set the value of a immutable global (`const`) will result in a `RuntimeError`
  ...
  // Changes made to global through exported functions will be reflected on the host side. 
