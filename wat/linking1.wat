(module
    ...
    (func (export "run")
        ;; Call into the other module to double our number, and we could print it here but for now we just drop it. 
        i32.const 2
        call $double
        drop

        ;; Our `data` segment initialized our imported memory, so let's print the string there now. 
        global.get $offset
        i32.const 14
        call $log
    )
